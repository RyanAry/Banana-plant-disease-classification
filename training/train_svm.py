...
def evaluate_model(model: Pipeline, features: pd.DataFrame, labels: pd.Series) -> tuple[np.ndarray, dict[str, float]]:
...
def main() -> None:
	...
	print(f"Total parameter combinations: {len(grid_search.param_grid)}")
	...
	model_path = model_directory / "svm_model.pkl"
	visualization_path = model_directory / "svm_training_results.png"
	metrics_path = model_directory / "svm_metrics.json"

	joblib.dump(
		{
			"model": best_model,
			"feature_columns": feature_columns,
			"classes": list(best_model.named_steps["svc"].classes_),
		},
		model_path,
	)
	...
	save_training_visualization(
		...
		y_test.to_numpy(),
		y_pred,
		list(best_model.named_steps["svc"].classes_),
		metrics,
		visualization_path,
	)
